class OurTimeLeft{
  List<String?> timeLeft(DateTime due){
    List<String?> retVal = List<String?>.filled(2, null);
...
Duration _timeUntilReveal = due.subtract(Duration(days: 7)).difference(DateTime.now());
 int _daysUntilReveal = _timeUntilReveal.inDays;
    int _hoursUntilReveal = _timeUntilReveal.inHours - (_daysUntil * 24);
    int _minUntilReveal = _timeUntilReveal.inMinutes - (_daysUntil * 24 * 60) - (_hoursUntil * 60);
  ...
  if (_daysUntilReveal > 0) {
    ...
    retVal[1] = _secUntilReveal.toString();
  }
